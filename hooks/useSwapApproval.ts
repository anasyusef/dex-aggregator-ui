...
function useSwapApprovalStates(
  ...
  const swapRouterAddress = chainId
    ...
    : undefined;
  const v2 = useApprovalStateForSpender(
    amountToApprove,
    v2RouterAddress,
    useIsPendingApproval
  );
  const v3 = useApprovalStateForSpender(
    amountToApprove,
    v3RouterAddress,
    useIsPendingApproval
  );
  const v2V3 = useApprovalStateForSpender(
    amountToApprove,
    swapRouterAddress,
    useIsPendingApproval
  );
...
export function useSwapApprovalOptimizedTrade(
  ...
  | Trade<Currency, Currency, TradeType>
  | undefined {
  const onlyV2Routes = trade?.routes.every(
    (route) => route.protocol === Protocol.V2
  );
  const onlyV3Routes = trade?.routes.every(
