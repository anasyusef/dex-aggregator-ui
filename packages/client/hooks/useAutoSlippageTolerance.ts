...
function guesstimateGas(
  ...
  if (!!trade) {
    return (
      100_000 +
      trade.swaps.reduce((memo, swap) => swap.route.pools.length + memo, 0) *
        30_000
    );
  }
  return undefined;
...
export default function useAutoSlippageTolerance(
  ...
  const nativeCurrencyPrice = useUSDCPrice(
    (trade && nativeCurrency) ?? undefined
  );
